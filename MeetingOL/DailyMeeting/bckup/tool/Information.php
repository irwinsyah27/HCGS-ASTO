<?
session_start();
if (($_SESSION[User]!='6104048') and ($_SESSION[User]!='8402003') and ($_SESSION[User]!='KB06005') and ($_SESSION[User]!='6103041') and ($_SESSION[User]!='KC10047')){
...
<form method="post" enctype="multipart/form-data" name="form1" target="_self">
  <div align="center">
    <p>&nbsp;    </p>
...
<table width=100% border=0 bgcolor=#DDDDDD>
  ...
  <tr>
    <td>Keterangan :</td>
    <td><textarea name=tKeterangan cols=45 rows=5 class=ui-widget-content ></textarea></td>
    <td>Attach.. :</td>
    <td><input name=File type=File class=ui-widget-content id=File /></td>
  </tr>
  <tr>
...
if(!$bEdit==''){
$Sel1 = mysql_query ("Select * from Information where NoInfo='$bEdit'") or die ("Terdapat kesalahan Update!");
...
$sC2=mysql_query("select Information.*, Departemen.NamaDept, Departemen.DeptHead, date(now()) as tglskrg, date(now())-date(DailyMeeting.tglupdate) as updateby, DailyMeeting.tglupdate,DailyMeeting.userupdate from DailyMeeting, Information, Departemen where DailyMeeting.NoDept=Departemen.NoDept and DailyMeeting.NoDept=Information.NoDept and Departemen.NamaDept='HRPGA' AND Information.OnDelete=0
...
while ($row = mysql_fetch_object($sC2))
{
		...
		<td align=Center><button id=lnk3 onClick=MM_openBrWindow('../HRPGA/lampiranData/E/$FileE','File','width=1000,height=800')>Attach</button></td>
	  </tr>"); }
?>
    </table>
</td>
...
// membaca nama file
   ...
   $destination_path = '../'.$_SESSION[Dept].'/lampiranData/E/';
   $result = 0;
   ...
   sleep(1);
   $nFile = Str_replace(' ','%20',$namaFile);
$update="UPDATE Information SET Keterangan='$tKeterangan',FileE='$nFile' WHERE NoInfo='$uNoInfo'";
...
if($bSimpanIn=='Simpan') {
$data1 = mysql_query ("select NoDept, (SELECT date(now()))as jam from Departemen where NamaDept='$NamaDept'");
...
// membaca nama file
   ...
   sleep(1);
   $nFile = Str_replace(' ','%20',$namaFile);
$insert="INSERT INTO Information (NoDept,Keterangan,FileE) VALUES('$dDept[NoDept]','$tKeterangan','$nFile')";
// ------ jalankan perintah SQL untuk memasukkan data ke tabel PKH
