...
Partial Class HRGA_dIjin1
    Inherits System.Web.UI.Page

    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Session("userid") = "" Then
            Session("Q") = "Cuti/dIjin1.aspx?n=" & Request.QueryString("n").ToString
    ...
    Sub UpdateAtasan()
        ...
        If (Not cs.IsStartupScriptRegistered(cstype, csname1)) Then
            ...
            cstext1 += "location='dIjin1.aspx?n=" & Request.QueryString("n").ToString & "';"
    ...
    Sub UpdateHR()
        ...
        With cmd
            ...
            .Parameters("@NoIjin").Value = Request.QueryString("n").ToString
            If Me.RadioValidasi1.Checked = True Or RadioValidasi2.Checked = True Then
    ...
    Sub DataKaryawan()
        ...
        With cmd
            .Connection = con2
            .CommandType = CommandType.Text
            .CommandText = "Select Nrp, SisaCuti From tblIjin Where Noijin = @Noijin"
            .Parameters.Add("@Noijin", SqlDbType.VarChar, 50)
            .Parameters("@Noijin").Value = Request.QueryString("n").ToString
            dr2 = .ExecuteReader
        End With
        If dr2.Read = True Then
           Me.lblSisaCuti.Text = dr2.Item("SisaCuti").ToString & " hari"
        End If	
    ...
    Sub DataIjin()
        ...
        If dr.Read = True Then
            ...
            Me.lblRequestor.Text = dr.Item("Requestor").ToString
            Me.lblTglCreated.Text = Format(CDate(dr.Item("Tanggal").ToString), "dd MMMM yyyy")
            ...
            If dr.Item("ValidasiHR").ToString <> "" Then
                If dr.Item("ValidasiHR").ToString = True And dr.Item("PotongCuti").ToString = False Then
                    Me.RadioValidasi1.Checked = True
                    Me.RadioValidasi2.Checked = False
                ElseIf dr.Item("ValidasiHR").ToString = True And dr.Item("PotongCuti").ToString = True Then
                ...
                ElseIf dr.Item("ValidasiHR").ToString = True And dr.Item("PotongOff").ToString = True Then
                    Me.RadioValidasi1.Checked = False
                    Me.RadioValidasi2.Checked = True
                    Me.RadioPotongCuti.Visible = True
                    Me.RadioPotongOff.Visible = True
        ...
        If InStr(1, Session("jabatan").ToString, "HCGS & FAT DEPT. HEAD") > 0 or InStr(1, Session("ndPosisi").ToString, "HCGS & FAT DEPT. HEAD") > 0 or InStr(1, Session("jabatan").ToString, "PLANT DEPT. HEAD") > 0 or InStr(1, Session("ndPosisi").ToString, "PLANT DEPT. HEAD") > 0 or InStr(1, Session("jabatan").ToString, "HRM & ENGINEERING DEPT. HEAD") > 0 or InStr(1, Session("ndPosisi").ToString, "HRM & ENGINEERING DEPT. HEAD") > 0 or InStr(1, Session("jabatan").ToString, "SM SITE SECT. HEAD") > 0 or InStr(1, Session("ndPosisi").ToString, "SM SITE SECT. HEAD") > 0 or InStr(1, Session("jabatan").ToString, "PAYROLL SITE OFFICER") > 0 or InStr(1, Session("ndPosisi").ToString, "PAYROLL SITE OFFICER") > 0 Or InStr(1, Session("jabatan").ToString, "SHE SITE DEPT. HEAD") > 0 or InStr(1, Session("ndPosisi").ToString, "SHE SITE DEPT. HEAD") > 0 Or InStr(1, Session("jabatan").ToString, "HCGS SECT. HEAD") > 0 or InStr(1, Session("ndPosisi").ToString, "HCGS SECT. HEAD") > 0 Then
            ...
            If Trim(dr.Item("Departemen").ToString) = Trim(Session("dept").ToString) Then
               Me.Panel3.Enabled = True
            Else If (InStr(1, Session("jabatan").ToString, "HRM & ENGINEERING DEPT. HEAD") > 0) Or (InStr(1, Session("ndPosisi").ToString, "HRM & ENGINEERING DEPT. HEAD"))  Then ' Penambahan Approval HRM ENG
                Me.Panel3.Enabled = True
            Else
        ...
        ElseIf Session("ndPosisi").ToString = "PROJECT MANAGER" or Session("Jabatan").ToString = "PROJECT MANAGER" Then
            Me.Panel3.Enabled = True
            'If Trim(dr.Item("Departemen").ToString) <> Trim(Session("dept").ToString) Then
            '    Me.Panel3.Enabled = False
            'Else
