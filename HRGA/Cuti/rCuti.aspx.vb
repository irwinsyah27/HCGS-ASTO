        ...
        If dr.Read = True Then
            ...
            Else
                If (dr.Item("Golongan").ToString) >= 1 And (dr.Item("Golongan").ToString) <= 14 Then
                    CType(Me.DetailsView1.Rows(0).FindControl("RangeValidator1"), RangeValidator).MaximumValue = 16
                    CType(Me.DetailsView1.Rows(0).FindControl("RangeValidator1"), RangeValidator).ErrorMessage = "Maximum : 16 hari. "
                Else 'If dr.Item("StatusPenerimaan").ToString = "KIRIMAN" Then
                    If (dr.Item("Golongan").ToString) >= 15 And (dr.Item("Golongan").ToString) <= 28 Then
                        If InStr(1, dr.Item("Jabatan").ToString, "DEPT.HEAD") > 0 Then
                        ...
                        Else
                            ...
                            CType(Me.DetailsView1.Rows(0).FindControl("RangeValidator1"), RangeValidator).ErrorMessage = "Cuti Lapangan yang bisa diambil maksimal : 14 hari. "
                        End If
                    End If
    ...
    Sub AwalCutiTahunan1()
        ...
        If CType(Me.DetailsView1.Rows(0).FindControl("txtAwal"), TextBox).Text <> "" Then
            ...
            If CType(Me.DetailsView1.Rows(0).FindControl("txtTahunan"), TextBox).Text <> "0" Or CType(Me.DetailsView1.Rows(0).FindControl("txtBesar"), TextBox).Text <> "0" Then
                ...
                'End If
                If CType(Me.DetailsView1.Rows(0).FindControl("txtPerjalanan"), TextBox).Text = 0 Then
                    CType(Me.DetailsView1.Rows(0).FindControl("txtAkhir"), TextBox).Text = Format(Format(temp1.AddDays(-1), "dd/MM/yyyy HH:mm:ss"))
                Else
                    CType(Me.DetailsView1.Rows(0).FindControl("txtAkhir"), TextBox).Text = Format(Format(temp1, "dd/MM/yyyy HH:mm:ss"))
                End If
            Else
                CType(Me.DetailsView1.Rows(0).FindControl("txtAkhir"), TextBox).Text = Format(CDate(CType(Me.DetailsView1.Rows(0).FindControl("txtAwal"), TextBox).Text).AddDays(CInt(CType(Me.DetailsView1.Rows(0).FindControl("txtLapangan"), TextBox).Text) - 1 + CInt(CType(Me.DetailsView1.Rows(0).FindControl("txtPerjalanan"), TextBox).Text)), "dd/MM/yyyy HH:mm:ss")
            End If
        End If
        SisaCutiTahunan()
    End Sub
