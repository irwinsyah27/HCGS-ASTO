...
Partial Class HRGA_SKL_SKaEl_Ha
    ...
    Protected Sub calawallbr_SelectionChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles calawallbr.SelectionChanged
        ...
        ' CEK KUOTA LEMBUR ##MODUL TAMBAHAN 2018-02-14

        con1.open()
            with cmd1
                .Connection = con1
                .CommandType = CommandType.Text
                .CommandText = "select count(AwalLembur) as jumlah from (SELECT a.NrpRequestor, b.Dept, a.awallembur from dbo.VW_SKL as a inner join Karyawan as b on a.NrpRequestor = b.Nrp)as a where convert(DATE,a.AWALLEMBUR) = @tgl AND a.Dept = @dept group by CONVERT(DATE,a.awallembur)"
                .Parameters.Add("@tgl", SqlDbType.Date)
                .Parameters.Add("@dept", SqlDbType.VarChar,15)
                .Parameters("@tgl").Value = Format((Me.calawallbr.SelectedDate), "yyyy-MM-dd").ToString
                .Parameters("@dept").Value = Session("Dept").ToString
                dr1 = .ExecuteReader
            End With
         ...
         con2.open()
            with cmd2
                .Connection = con2
                .CommandType = CommandType.Text
                .CommandText = "Select kuota_lembur from Tbl_kuotaLemburDept where dept = @dept"
                .Parameters.Add("@dept", SqlDbType.VarChar,15)
                .Parameters("@dept").Value = Session("Dept").ToString
                dr2 = .ExecuteReader
            End With
            ...
            If dr1.Read = True  Then  
                if dr2.Read = True Then
                    If dr1.Item("jumlah") >= dr2.item("kuota_lembur") Then
				    'If 1 < 0 Then
                        If (Not cs.IsStartupScriptRegistered(cstype, csname1)) Then
                            Dim cstext1 As String = "alert('Mohon maaf, Kuota Lembur Departemen " & Session("Dept").ToString & " maksimal adalah " & dr2.item("kuota_lembur") & ",kuota lembur saat ini sudah habis !');"
                    ...
                    If (Not cs.IsStartupScriptRegistered(cstype, csname1)) Then
                            Dim cstext1 As String = "alert('Terjadi kesalahan : Data Kuota Lembur tidak ditemukan !');"
                            cs.RegisterStartupScript(cstype, csname1, cstext1, True)
                        Exit Sub
                    End If
			...
			Else		
				If dr2.Read = True Then
                    If 0 >= dr2.item("kuota_lembur") Then
				    'If 1 < 0 Then
                        If (Not cs.IsStartupScriptRegistered(cstype, csname1)) Then
                            Dim cstext1 As String = "alert('Mohon maaf, Kuota Lembur Departemen " & Session("Dept").ToString & " maksimal adalah " & dr2.item("kuota_lembur") & ",kuota lembur saat ini sudah habis !');"
                            cs.RegisterStartupScript(cstype, csname1, cstext1, True)
    ...
    Protected Sub txtnrp_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtnrp.TextChanged
        ...
        With cmd1
            .Connection = con1
            .CommandType = CommandType.Text
            .CommandText = "SELECT nrp,tanggal,[in],[out] From dbo.lembarKerja where nrp = @Nrp and tanggal = @Tanggal"
            .Parameters.Add("@Nrp", SqlDbType.VarChar, 10)
			.Parameters.Add("@Tanggal", SqlDbType.Date)
            .Parameters("@Nrp").Value = txtnrp.Text
			.Parameters("@Tanggal").Value = Format(CDate(Me.txttglawal.Text), "yyyy-MM-dd")
            dr1 = .ExecuteReader
        End With
		
		If dr1.Read = True Then
			
			If dr1.Item("in").ToString = "" Then		
				  Dim csname1 As String = "PopupScript"
					Dim csname2 As String = "ButtonClickScript"
					Dim cstype As Type = Me.GetType()
					Dim cs As ClientScriptManager = Page.ClientScript
				 ...
				 If (Not cs.IsStartupScriptRegistered(cstype, csname1)) Then
							Dim cstext1 As String = "alert('Absen IN untuk NRP tersebut kosong, mohon di revisi dulu !');"
							cs.RegisterStartupScript(cstype, csname1, cstext1, True)
							Exit Sub
				 End If	
			End If
			
			If dr1.Item("out").ToString = "" Then	
				...
				Dim cs As ClientScriptManager = Page.ClientScript
				 ...
				 If (Not cs.IsStartupScriptRegistered(cstype, csname1)) Then
							Dim cstext1 As String = "alert('Absen OUT untuk NRP tersebut kosong, mohon di revisi dulu !');"
							cs.RegisterStartupScript(cstype, csname1, cstext1, True)
							Exit Sub
				 End If
		...
		Else
			...
			Dim cs As ClientScriptManager = Page.ClientScript
			 ...
			 If (Not cs.IsStartupScriptRegistered(cstype, csname1)) Then
							Dim cstext1 As String = "alert('Terjadi Kesalah !!!');"
							cs.RegisterStartupScript(cstype, csname1, cstext1, True)
							Exit Sub
			 End If
    ...
    Protected Sub btnsend_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnsend.Click
		...
		Dim LemburAwal As DateTime = Me.TxtAwalSKL.Text
		Dim LemburAkhir As DateTime = Me.TxtAkhirSKL.Text
		Dim SelisihJam As TimeSpan = (LemburAkhir - LemburAwal)
		Dim JamLembur As String = SelisihJam.ToString()
		Dim MaxLembur As String = "01:30:00"
		
		If  JamLembur > MaxLembur  Then
			Dim cstext1 As String = "alert('Max Lembur 1.5 Jam');"
            cstext1 += "self.close();"
            cs.RegisterStartupScript(cstype, csname1, cstext1, True)
		...
		'DateDiff(DateInterval.Minute, Me.TxtAwalSKL.Text, Me.TxtAkhirSKL.Text)
