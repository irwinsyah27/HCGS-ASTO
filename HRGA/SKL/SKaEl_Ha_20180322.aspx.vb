    ...
    Protected Sub calawallbr_SelectionChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles calawallbr.SelectionChanged
        ...
        con1.open()
            with cmd1
                ...
                .CommandText = " select CONVERT(DATE,a.tglcreated), COUNT(a.tglCreated) as jumlah from (SELECT a.NrpRequestor, b.Dept, a.TglCreated from Tbl_SKL1 as a inner join Karyawan as b on a.NrpRequestor = b.Nrp) as a where convert(DATE,a.TglCreated) = @tgl and a.Dept = @dept group by CONVERT(DATE,a.TglCreated)"
