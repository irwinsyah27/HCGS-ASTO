...
<table width=100% border=0 bgcolor=#DDDDDD>
  ...
  <tr>
    <td>Number</td>
    <td>:
      <input name=tNumber type=text class=ui-widget-content size=57 value='$dEd[NumberC]' />    </td>
    <td>Result</td>
    <td>:<strong>
      <input name=tResult type=text class=ui-widget-content size=57 value='$dEd[ResultC]' />
    </strong></td>
  </tr>
  <tr>
    <td colspan=4><div align=center class=ui-widget-content>
...
<table width=100% border=0 bgcolor=#DDDDDD>
  <tr>
    ...
    <td width=40% class=ui-widget-header><div align=center>Case</div></td>
...
$sC2=mysql_query("select C2.*, SUM(C2.ResultC) as TResultC, SUM(C2.NumberC) as TNumberC, Departemen.NamaDept, Departemen.DeptHead, date(now()) as tglskrg, date(now())-date(DailyMeeting.tglupdate) as updateby, DailyMeeting.tglupdate,DailyMeeting.userupdate from DailyMeeting, C2, Departemen where DailyMeeting.NoDept=Departemen.NoDept and DailyMeeting.NoDept=C2.NoDept and Departemen.NamaDept='HRPGA'and C2.tipe='C' AND C2.OnDelete=0 group by NoC2") or die ("Terdapat kesalahan pada perintah SQL!");
while ($row = mysql_fetch_object($sC2))
{
		...
		$Result=$row->Result;
		$Remark=$row->Remark;
		$TNumberC=$row->TNumberC;
		$TResultC=$row->TResultC;
		$NumberC=$row->NumberC;
		$ResultC=$row->ResultC;
		echo ("
		<tr>
		...
		echo ("
		<td align=Center>$NumberC</td>
		<td align=Center>$ResultC</td>
		...
		<td align=Center><strong>$TNumberC</td>
		<td align=Center><strong>$TResultC</td>
		<td></td>
	  </tr>
	</table>");
?></td>
...
<?
if($bSimpan=='Simpan') {
$update="UPDATE C2 SET Complaint='$tComplaint',NumberC='$tNumber',ResultC='$tResult',Remark='$tRemark' WHERE NoC2='$uNoC2'";
...
$insert="INSERT INTO C2 (NoDept,Tipe,Complaint,NumberC,ResultC,Remark) VALUES('$dDept[NoDept]','C','$tComplaint','$tNumber','$tResult','$tRemark')";
// ------ jalankan perintah SQL untuk memasukkan data ke tabel PKH
