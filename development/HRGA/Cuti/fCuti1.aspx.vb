...
Partial Class _fCuti1
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If dr.Read = True Then
            ...
            '--------
			If Session("Admin").ToString <> "Admin" AND Session("Jabatan").ToString <> "KORLAP" Then
    ...
    Protected Sub btSave_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btSave.Click
		...
		Dim maxCuti As Integer = 9
		Dim maxOnsite As Integer = 9
		...
		con1.open()
			 with cmd1
				.Connection = con1
				.CommandType = CommandType.Text
				.CommandText = "SELECT count(*) as jumlah FROM [tblSutu] WHERE KEPERLUAN = 'CUTI' AND AWAL = @tgl AND (Status = 'Complete' or status = 'HRPGA Dept. Head' or Status = 'HRPGA Sect. Head') and nrp not like 'kr%'"
		...
		con2.open()
			 with cmd2
				.Connection = con2
				.CommandType = CommandType.Text
				.CommandText = "SELECT count(*) as jumlah FROM [tblSutu] WHERE KEPERLUAN = 'CUTI' AND AKHIR = @tgl AND  (Status = 'Complete' or status = 'HRPGA Dept. Head' or Status = 'HRPGA Sect. Head') and nrp not like 'kr%'"
				.Parameters.Add("@tgl", SqlDbType.Date)
		...
		If jumlahCuti > maxCuti And Me.txtNrp.Text.Substring(0, 2).ToUpper <> "KR" And Session("Jabatan").ToString <> "PERSONNEL SITE OFFICER"  Then 
			...
			If (Not cs.IsStartupScriptRegistered(cstype, csname1)) Then
				Dim cstext2 As String = "alert('Mohon maaf kuota sarana cuti sudah penuh, Silahkan reschedule cuti !');"
			   cs.RegisterStartupScript(cstype, csname1, cstext2, True)
			End If
			
		Else If jumlahOnsite > maxOnsite And Me.txtNrp.Text.Substring(0, 2).ToUpper <> "KR" And Session("Jabatan").ToString <> "PERSONNEL SITE OFFICER"  Then
			...
			If (Not cs.IsStartupScriptRegistered(cstype, csname1)) Then
				Dim cstext2 As String = "alert('Mohon maaf kuota sarana onsite sudah penuh, Silahkan reschedule cuti !');"
			   cs.RegisterStartupScript(cstype, csname1, cstext2, True)  
			End If
