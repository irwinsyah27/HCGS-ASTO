    ...
    function ResizeMe(idGrid, ExtraWidth, ExtraHeight)
    {
		if(window.frameElement != null)
		{
    		if(window.frameElement.tagName == "IFRAME")
			{
				...
				window.frameElement.width = GridViewHelper.CalcWidth(document.all.ctl00_ContentPlaceHolder1_GridView2)+ExtraWidth;
		...
		var Parent = ctl00_ContentPlaceHolder1_GridView2.offsetParent;
		// make the parent really wide so that no columns will wrap
		Parent.style.width = "10000px";
		// calcuate the real width
		var RealWidth = GridViewHelper.CalcWidth(document.all.ctl00_ContentPlaceHolder1_GridView2)+100;
		// set the parent width back to nothing	
		Parent.style.width = "";
		//set the grid to the size it needs to be
		ctl00_ContentPlaceHolder1_GridView2.width=""+RealWidth;			
	}
