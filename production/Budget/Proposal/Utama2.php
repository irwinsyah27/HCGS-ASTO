...
$q6 = mssql_query("SELECT [Acount] AS ID
      ,[Descr] AS PIT
  FROM [BudgetSystem].[dbo].[v_COA_L3]
  where Descr LIKE 'PIT -%'");
...
?>
          <table width="100%" border="0" align="center">
            <tr>
              <td><div class="style52">
                  <table cellspacing="0" cellpadding="0">
                    <tr height="20">
                      <td colspan="5" class="style57">PT.   KALIMANTAN PRIMA PERSADA</td>
                    </tr>
                    <tr height="20">
                      <td height="20" colspan="2" class="style55">BUDGET COST AND CASH   OUT</td>
                      <td width="4">&nbsp;</td>
                      <td colspan="3"></td>
                    </tr>
                    <tr height="20">
                      <td width="286" height="20" class="style55">PERIODs</td>
                      <td colspan="5" class="style55">: <?php echo $pick_date ; ?> s/d <?php echo $pick_date2 ; ?> </td>
                    </tr>
                    <tr height="20">
...
FROM         (SELECT     Tbl.ID, Tbl.Periode, Tbl.Start, Tbl.Stop, Tbl.IDDept, Tbl.IDDeptAdd, Tbl.DeptAdd, dbo.v_Proposal.Dept, dbo.v_Proposal.Kategori, dbo.v_Proposal.Acount, 
                      dbo.v_Proposal.Descr, dbo.v_Proposal.List, ISNULL(Tbl.Nominal, 0) AS Nominal, ISNULL(Tbl.StatusClosing, 0) AS StatusClosing
FROM         (SELECT     dbo.Proposal.ID, CAST(DATEPART(MONTH, dbo.ref_Closing.Periode) AS VARCHAR) + '/' + CAST(DATEPART(YEAR, dbo.ref_Closing.Periode) 
                                              ...
                                              dbo.v_COA_L4.Descr AS Dept, dbo.ref_Dept.NamaDept AS DeptAdd, dbo.Proposal.DeptAdd AS IDDeptAdd, dbo.v_COA_L5.Kategori, 
                                              dbo.Proposal.Acount, dbo.v_COA_L5.Descr, dbo.ref_Acounts.Descr AS List, SUM(dbo.Proposal.Nominal) AS Nominal, 
                                              ISNULL(dbo.ref_Closing.Status, 0) AS StatusClosing, dbo.v_COA_L4.Acount AS IDDept
                       ...
                       GROUP BY dbo.Proposal.Acount, dbo.Proposal.Nominal, dbo.v_COA_L5.Descr, dbo.v_COA_L5.Kategori, dbo.ref_Closing.Periode, 
                                              ...
                                              dbo.Proposal.ID, dbo.v_COA_L4.Acount, dbo.Proposal.DeptAdd) AS Tbl LEFT OUTER JOIN
...
Where Start = '".$pick_date."' AND Stop = '".$pick_date2."' AND IDDept = '".$mmDept."' AND Acount ='".$mmAc."' AND IDDeptAdd = $SDeptAdd
GROUP BY Periode, Kategori, Acount, Descr, Dept
 ");
 
 $selProp2=mssql_query("SELECT     Periode, SUM(Nominal) AS Nominal, Dept
FROM         (SELECT     Tbl.ID, Tbl.Periode, Tbl.Start, Tbl.Stop, Tbl.IDDept, Tbl.IDDeptAdd, Tbl.DeptAdd, dbo.v_Proposal.Dept, dbo.v_Proposal.Kategori, dbo.v_Proposal.Acount, 
                      dbo.v_Proposal.Descr, dbo.v_Proposal.List, ISNULL(Tbl.Nominal, 0) AS Nominal, ISNULL(Tbl.StatusClosing, 0) AS StatusClosing
FROM         (SELECT     dbo.Proposal.ID, CAST(DATEPART(MONTH, dbo.ref_Closing.Periode) AS VARCHAR) + '/' + CAST(DATEPART(YEAR, dbo.ref_Closing.Periode) 
                                              AS VARCHAR) AS Periode, dbo.Proposal.Start, dbo.Proposal.Stop, DATEDIFF(DAY, dbo.Proposal.Start, dbo.Proposal.Stop) AS TotalHari, 
                       ...
                       GROUP BY dbo.Proposal.Acount, dbo.Proposal.Nominal, dbo.v_COA_L5.Descr, dbo.v_COA_L5.Kategori, dbo.ref_Closing.Periode, 
                                              ...
                                              dbo.Proposal.ID, dbo.v_COA_L4.Acount, dbo.Proposal.DeptAdd) AS Tbl LEFT OUTER JOIN
                      dbo.v_Proposal ON Tbl.Dept = dbo.v_Proposal.Dept AND Tbl.Acount = dbo.v_Proposal.Acount AND Tbl.List = dbo.v_Proposal.List) AS Tbl
Where Start = '".$pick_date."' AND Stop = '".$pick_date2."' AND IDDept = '".$mmDept."' AND IDDeptAdd = $SDeptAdd
GROUP BY Periode, Dept
 ");
$rowIBS = mssql_fetch_array($selIBS);
$rowProp = mssql_fetch_array($selProp);
$rowProp2 = mssql_fetch_array($selProp2);
...
$TIBS = number_format($rowIBS[IBS],0,",","."); 
	  ...
	  ?><span class="style5"> <span class="style56">-----------</span>
            ...
            <tr>
              <td><table width="100%" border="0" bgcolor="#CCCCFF">
                  <tr>
                          ...
                          <tr>
                            <td width="197">Periode :
                              <label><span class="style2">
                                <input name="pick_date" type="text" id="pick_date" value="<?php if($pick_date==''){echo date('Y-m-d');} else echo $pick_date; ?>" size="12" />
                            ...
                            <td width="174"><label>s/d : <span class="style2">
                              <input name="pick_date2" type="text" id="pick_date2" value="<?php if($pick_date2==''){echo date('Y-m-d');} else echo $pick_date2; ?>" size="12" />
				  ...
				  Periode) AS VARCHAR) AS Periode from Proposal Where Start = '".$pick_date."' AND Stop ='".$pick_date2."'
				   ...
				   $rowBln = mssql_fetch_array($selBln);
					?>
                                <input name="fClosing" type="text" id="fClosing" value="<?php echo $rowBln[Periode]; ?>">
                                <label></label></td>
                            ...
                            <td width="58"><input name="Submit2" type="submit" value="refresh"></td>
                          </tr>
                        </table>
                      <a><strong>Tampilkan Report :</strong>
                        <input name="View" type="checkbox" id="View" value="1" <?php if ($View) {echo "checked=1";} ?> />
                      </a> </div>
                        <div align="left"></div></td>
                  </tr>
                  <tr>
                    <td bgcolor="#CCFFFF"><input name="hID" type="hidden" id="hID" value="<?php echo $bEdit; ?>">
                        <?php if($bIns=='Insert'){ echo ("
...
SELECT     Tbl.ID, Tbl.Periode, Tbl.Start, Tbl.Stop, Tbl.IDDept, Tbl.IDDeptAdd, Tbl.DeptAdd, dbo.v_Proposal.Dept, dbo.v_Proposal.Kategori, dbo.v_Proposal.Acount, 
                      dbo.v_Proposal.Descr, dbo.v_Proposal.List, ISNULL(Tbl.Nominal, 0) AS Nominal, ISNULL(Tbl.StatusClosing, 0) AS StatusClosing
FROM         (SELECT     dbo.Proposal.ID, CAST(DATEPART(MONTH, dbo.ref_Closing.Periode) AS VARCHAR) + '/' + CAST(DATEPART(YEAR, dbo.ref_Closing.Periode) 
                                              AS VARCHAR) AS Periode, dbo.Proposal.Start, dbo.Proposal.Stop, DATEDIFF(DAY, dbo.Proposal.Start, dbo.Proposal.Stop) AS TotalHari, 
                                              dbo.v_COA_L4.Descr AS Dept, dbo.ref_Dept.NamaDept AS DeptAdd, dbo.Proposal.DeptAdd AS IDDeptAdd, dbo.v_COA_L5.Kategori, 
...
'0'+CAST(DATEPART(DAY, Stop) AS VARCHAR) ELSE CAST(DATEPART(DAY, Stop) AS VARCHAR) END)
 ...
 (CAST(DATEPART(YEAR, Periode) AS VARCHAR) + '-' + 
CASE WHEN len(CAST(DATEPART(MONTH, Periode) AS VARCHAR)) = 1 THEN
'0'+CAST(DATEPART(MONTH, Periode) AS VARCHAR) ELSE CAST(DATEPART(MONTH, Periode) AS VARCHAR) END
 + '-' +
CASE WHEN len(CAST(DATEPART(DAY, Periode) AS VARCHAR)) = 1 THEN
'0'+CAST(DATEPART(DAY, Periode) AS VARCHAR) ELSE CAST(DATEPART(DAY, Periode) AS VARCHAR) END)
 AS Periode
...
$Peri=$rowRef[Periode];
$insert="INSERT INTO Proposal (Periode,Start,Stop,Dept,DeptAdd,Acount,List,Nominal) VALUES('$Peri','$pick_date','$pick_date2','$mmDept',$SDeptAdd,'$mmAc','$mmLi','$mNominal')";
...
FROM         (SELECT     dbo.Proposal.ID, CAST(DATEPART(MONTH, dbo.ref_Closing.Periode) AS VARCHAR) + '/' + CAST(DATEPART(YEAR, dbo.ref_Closing.Periode) 
                       ...
                       GROUP BY dbo.Proposal.Acount, dbo.Proposal.Nominal, dbo.v_COA_L5.Descr, dbo.v_COA_L5.Kategori, dbo.ref_Closing.Periode, 
                                              ...
                                              dbo.Proposal.ID, dbo.v_COA_L4.Acount, dbo.Proposal.DeptAdd) AS Tbl LEFT OUTER JOIN
                      dbo.v_Proposal ON Tbl.Dept = dbo.v_Proposal.Dept AND Tbl.Acount = dbo.v_Proposal.Acount AND Tbl.List = dbo.v_Proposal.List) AS Tbl
					  Where (Start = '".$pick_date."' AND Stop = '".$pick_date2."') AND IDDept = '".$mmDept."' AND Kategori = '".$mmKat."' AND Acount = '".$mmAc."' AND IDDeptAdd = $SDeptAdd
				  ")
...
echo("<td bgcolor=#336699><div align=center><span class=style2>
							  ...
							  <select name=mmPIT id=mmPIT onChange=this.form.submit()>");
							  echo "<option value=></option>";
							  echo "<option value= '$row6[PIT]' "; if ($row6[PIT]==$mmPIT){echo"selected=selected";} echo">$row6[PIT]
							  </option>";


							  while ($row6 = mssql_fetch_array($q6)){
							  echo "<option value= '$row6[PIT]' "; if ($row6[PIT]==$mmPIT){echo"selected=selected";} echo">$row6[PIT]
							  </option>"; }
...
?></td>
                  ...
                  <tr>
                    <td bgcolor="#CCFFFF"><table width="100%" border="0">
                        <tr>
                          <td width="877"><div align="right" class="style55"> Total : </a></div></td>
                          <td width="107"><a class=style55><a href="#"> <?php echo strval($rowIBS[IBS])-strval($rowProp[Nominal]) ; 
			  ?></td>
                        </tr>
                    </table></td>
                  </tr>
