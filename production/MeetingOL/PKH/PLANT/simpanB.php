...
if(!$mKepada==$_SESSION[NamaSec]){
	...
	require("../../Tools/config.php");
	//set sudah dibaca = Y kalau sudah dibaca
	$insert = mysql_query("INSERT INTO tabel_pesan VALUES(null,'$DateTimeSkrg''$d1[jam]','PKH','HeadPLANT','$noPK[NoPKH]','N')"); }
} else {
  // membaca nama file
  ...
  $insert="INSERT INTO PKH (TglPK,NoSec,Sejak,Lokasi,Instruksi_Kerja,Laporan_Pelaksanaan,Status,Lampiran,File,Type,TglUpdate)
  VALUES('$TglSkrg','$d1[NoSec]','$mSejak','$mLokasi','$mInstruksi','$mLaporan','$mStatus','$namaFile','$isiFile','$typeFile','$TglSkrg')";
  // ------ jalankan perintah SQL untuk memasukkan data ke tabel PKH
  $hasil =@mysql_query ($insert) or die ("Terdapat Kesalahan SQL");
  // ------ jalankan perintah SQL untuk memasukkan data ke tabel PKH
