...
$sel1=mysql_query("Select Departemen.DeptHead from Departemen where Departemen.NoDept='1'")or die ("Terdapat kesalahan pada perintah SQL!");
}
                      ...
                      <tr>
                        ...
                        <td width="14%"><div align="right">
                            <select name="Jmenu" onChange="MM_jumpMenu(this,'Report','width=1000,height=590,scrollbars=yes')">
                              <option value="#"> </option>
							  <option value="../../../ReportServer/PKH/HRPGA/Open_PerPerioderpt.php">PKH_Open</option>
                              <option value="../../../ReportServer/PKH/HRPGA/Close_PerPerioderpt.php">PKH_CloseOff</option>
                              <option value="../../../ReportServer/PKH/HRPGA/Diagram_Secctb.php">Chart_StatusLaporan</option>
                            </select>
                        </div></td>
                      </tr>
                  </table></td>
...
// ----- SQL data PKH
if ($Pesan=='') {
$sel2=mysql_query("select PKH.nopkh, PKH.TglPK, SecDept.NamaSec, PKH.lokasi, PKH.sejak, PKH.instruksi_kerja, PKH.laporan_pelaksanaan, PKH.status, PKH.kondisi, PKH.lampiran, PKH.file, PKH.type, PKH.lampiran, date(now()) as tglskrg, date(now())-date(PKH.tglupdate) as updateby, PKH.tglupdate,PKH.userupdate from PKH, SecDept, Departemen where PKH.NoSec=SecDept.NoSec and SecDept.NoDept=Departemen.NoDept and PKH.On<>0 and SecDept.NamaSec='$mKepada' and Departemen.DeptHead='$mDeptHead' and PKH.TglPK<='$pick_date' order by PKH.Sejak limit $curRec,$maxRec")
or die ("Terdapat kesalahan pada perintah SQL!"); } else {
$sel2=mysql_query("select PKH.nopkh, PKH.TglPK, SecDept.NamaSec, PKH.lokasi, PKH.sejak, PKH.instruksi_kerja, PKH.laporan_pelaksanaan, PKH.status, PKH.kondisi, PKH.lampiran, PKH.file, PKH.type, PKH.lampiran, date(now()) as tglskrg, date(now())-date(PKH.tglupdate) as updateby, PKH.tglupdate,PKH.userupdate from PKH, SecDept, Departemen where PKH.NoSec=SecDept.NoSec and SecDept.NoDept=Departemen.NoDept and Departemen.DeptHead='$d1[DeptHead]' and PKH.TglPK<= Date(Now()) and nopkh='$Pesan' order by PKH.Sejak limit $curRec,$maxRec")
...
while ($row = mysql_fetch_object($sel2))
{
		...
		$kondisi=$row->kondisi;
		$type=$row->type;
		$lampiran='Pendukung/data'.$mKepada.'/'.$row->lampiran;
		...
		echo("<tr><div align=center><td bgcolor=#666666 class=style5>$nomor</td>
			...
			if($kondisi=='1'){	
				if(!$type==''){
				echo ("<td bgcolor=#666666 class=stylebtKondisi1 align=center ><img src=../Images/lampiran.gif href=# onClick=MM_openBrWindow('$lampiran','File','width=600,height=390')></td>"); } else {
				echo("<td bgcolor=#666666 class=stylebtKondisi1></td>"); }
			} else if($kondisi=='2'){
				if(!$type==''){
				echo ("<td bgcolor=#666666 class=stylebtKondisi2 align=center ><img src=../Images/lampiran.gif href=# onClick=MM_openBrWindow('$lampiran','File','width=600,height=390')></td>"); } else {
				echo("<td bgcolor=#666666 class=stylebtKondisi2></td>"); }
			} else {
				if(!$type==''){
				echo ("<td bgcolor=#666666 class=style5 align=center ><img src=../Images/lampiran.gif href=# onClick=MM_openBrWindow('$lampiran','File','width=1000,height=800')></td>"); } else {
				echo("<td bgcolor=#666666 class=style5></td>"); }
			}
			if($updateby<='1'){
              ...
              <tr>
                <td bgcolor="#666666"><div align="right">
				  ...
				  // get total Record from your table in database
					if ($Pesan=='') {$tot=mysql_query("select count(NoPKH) as tot from PKH, SecDept, Departemen where PKH.NoSec=SecDept.NoSec and SecDept.NoDept=Departemen.NoDept and PKH.On<>0 and SecDept.NamaSec='$mKepada' and Departemen.DeptHead='$mDeptHead' and PKH.TglPK<='$pick_date'");	
