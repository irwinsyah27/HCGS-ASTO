...
//-->
      ...
      <tr>
        ...
        <div align="center"><span class="style3">Laporan Harian ke PM</span>
        </div>
            ...
            <tr>
              <td width="549" bgcolor="#FFFFFF"><table width="400" border="0" align="center">
                  <tr>
                    ...
                    <td width="174"><input name="mNo" type="text" id="mNo" value="<? $SNo=mssql_query("Select No from Lap_PM WHERE Tanggal BETWEEN '$pick_date' and '$date1' order by no DESC"); $sNo=mssql_fetch_array($SNo); if($sNo[No]=='') {echo 'LH/091/..-../SEC-KPP/20..';} else {echo $sNo[No];} ?>" size="25" /></td>
                  </tr>
                </table>
                  <? if(!$bEdit==''){
$Sel1 = mssql_query ("select Id, No, Shift, CAST(CAST(datepart(HOUR,[Tanggal]) as varchar)+':'+CAST(datepart(MINUTE,[Tanggal]) as varchar) AS varchar) AS Tanggal, Kegiatan, Keterangan from Lap_PM where Id='$bEdit'") or die ("Terdapat kesalahan Update!");
...
if($bUpdate=='Update') {
$Tgl= $tJam.':000';
$update="UPDATE Lap_PM SET No='$mNo', Tanggal='$pick_date $Tgl', Shift='$tShift', Kegiatan='$tKegiatan', Keterangan='$tKeterangan' WHERE Id='$hID'";
...
if($bSave=='Save') {
$Tgl= $tJam.':000';
$insert="INSERT INTO Lap_PM (No, Tanggal, Shift, Kegiatan, Keterangan) VALUES('$mNo','$pick_date $Tgl','$tShift','$tKegiatan','$tKeterangan')";
...
if(!$bDelete=='') {
$delete="DELETE From Lap_PM WHERE Id='$bDelete'";
...
");} ?></td>
          ...
          <table width="100%" border="0" align="center">
            ...
            <tr>
              <td><? 
$sel2=mssql_query("select Id, No, Shift, CAST(CAST(datepart(HOUR,[Tanggal]) as varchar)+':'+CAST(datepart(MINUTE,[Tanggal]) as varchar) AS varchar) AS Tanggal, Kegiatan, Keterangan from Lap_PM where Shift=1 and Tanggal BETWEEN '$pick_date' and '$date1'");
...
?></td>
            ...
            <tr>
              <td><?
$sel2=mssql_query("select Id, No, Shift, CAST(CAST(datepart(HOUR,[Tanggal]) as varchar)+':'+CAST(datepart(MINUTE,[Tanggal]) as varchar) AS varchar) AS Tanggal, Kegiatan, Keterangan from Lap_PM where Shift=2 and Tanggal BETWEEN '$pick_date' and '$date1'");
$sel3=mssql_query("Select COUNT(NRP) as [Tot] from Petugas");
$dSum=mssql_fetch_array($sel3);
// ------ buat tampilan tabel
echo("<table width=100%  border=0 bgcolor=#99CCFF>");
